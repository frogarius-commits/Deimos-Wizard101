...
def read_config(config_name : str):
	...
	friend_teleport_key = parser.get('hotkeys', 'friend_teleport', fallback='F11')
	...
	# Auto GCR Tokens Defaults
	global wood_tokens_enabled
	global silver_tokens_enabled
	global gold_tokens_enabled
	global wood_token
	global silver_token
	global gold_token
	wood_tokens_enabled = False
	silver_tokens_enabled = False
	gold_tokens_enabled = False
	wood_token = 'None'
	silver_token = 'None'
	gold_token = 'None'
...
gcr_tokens_task: asyncio.Task = None
						...
						for entity in entities_list:
							...
							for drum in excluded_drums:
								if await are_xyzs_within_threshold(entity_pos, drum):
									check = False
									if debug:
										logger.debug(f'Invalid lightpad, staying in place')
									break
									#client_pos = await foreground_client.body.position()
									#await WorldsCollideTP(foreground_client, client_pos, static_body_radius=0.0)
							if check:
								entities.append(entity)
...
async def main():
	...
	async def wood_chest_hotkey():
		await wood_chest_tp(foreground_client, "peanits", debug=True)
    
	async def silver_chest_hotkey():
		await silver_chest_tp(foreground_client, "peanits", debug=True)
    
	async def gold_chest_hotkey():
		await gold_chest_tp(foreground_client, "peanits", debug=True)
    
	async def tracy_tp_hotkey():
		await tracy_tp(foreground_client, "peanits", debug=True)
    
	async def snake_tp_hotkey():
		await entity_tp_helper(foreground_client, "peanits", "snake_pu", debug=True)
    
	async def spider_tp_hotkey():
		await entity_tp_helper(foreground_client, "peanits", "spider_pu", debug=True)
    
	async def crane_tp_hotkey():
		await entity_tp_helper(foreground_client, "peanits", "crane_pu", debug=True)
    
	async def butterfly_tp_hotkey():
		await entity_tp_helper(foreground_client, "peanits", "butterfly_pu", debug=True)

	async def tree_tp_hotkey():
		await entity_tp_helper(foreground_client, "peanits", "tree_pu", debug=True)
	...
	async def auto_chest_loop(chest_type):
		try:
			logger.debug(f'Auto filling {chest_type} Chest')
			while True:
				match chest_type:
					case 'Mana':
			...
			logger.debug(f'Auto {chest_type} Chest task cancelled.')

	async def auto_chest_hotkey(chest_type):
		global auto_chest_task
		global gui_send_queue
		...
		if auto_chest_task is not None and not auto_chest_task.cancelled():
			auto_chest_task.cancel()
			auto_chest_task = None

			logger.debug(f'Auto {chest_type} Chest Disabled')
			gui_send_queue.put(deimosgui.GUICommand(deimosgui.GUICommandType.UpdateWindow,("AutoChestStatus", "Disabled")))
			return

		logger.debug(f'Auto {chest_type} Chest Enabled')
		gui_send_queue.put(deimosgui.GUICommand(deimosgui.GUICommandType.UpdateWindow,("AutoChestStatus", "Enabled")))
		auto_chest_task = asyncio.create_task(auto_chest_loop(chest_type))
	...
	async def power_star_hotkey():
		if foreground_client:
			logger.debug(f'Collecting Power Star.')
			await entity_tp_helper(foreground_client, "Temp", "raid_powersource", debug=False)
			await is_visible_by_path(foreground_client, ['WorldView', 'NPCRangeWin', 'imgBackground'])
			await foreground_client.send_key(Keycode.X, 0.2)
		...
		match raid:
			case 'vvr':
				if vvr_drums_task is not None and not vvr_drums_task.cancelled():
					...
					logger.debug("VVR Drums Disabled")
					gui_send_queue.put(deimosgui.GUICommand(deimosgui.GUICommandType.UpdateWindow,("VVRDrumsStatus", "Disabled")))
					return

				logger.debug("VVR Drums Enabled")
				gui_send_queue.put(deimosgui.GUICommand(deimosgui.GUICommandType.UpdateWindow,("VVRDrumsStatus", "Enabled")))
				vvr_drums_task = asyncio.create_task(drums_loop(client))

			case 'csr':
				if csr_drums_task is not None and not csr_drums_task.cancelled():
					csr_drums_task.cancel()
					csr_drums_task = None
	...
	async def drums_loop(clients: list[Client]):
		try:
			...
			while True:
				entities_list = await SprintyClient(clients).get_base_entities_with_vague_name("lightpad")
				entities = []
				last_drum_location: XYZ = (0.0, 0.0, 0.0)
				for entity in entities_list:
					entity_pos: XYZ = await entity.location()
					check = True
					for drum in excluded_drums:
						if await are_xyzs_within_threshold(entity_pos, drum):
							...
							break
					if check:
						if entity_pos != last_drum_location:
							entities.append(entity)
				if entities:
					entity = await SprintyClient(clients).find_closest_of_entities(entities)
					entity_pos: XYZ = await entity.location()
					player_pos = await clients.body.position()
					if entity_pos == player_pos:
						break
					else:
						await clients.teleport(entity_pos)
				await asyncio.sleep(0.1)

		except asyncio.CancelledError:
			logger.debug("Drums task cancelled.")

	# CSR Hotkeys

	async def location_teleport_hotkey(location):
		if foreground_client:
			logger.debug(f'Teleporting to {location}')
			match location:
				case 'Pyramid':
					await foreground_client.teleport(XYZ(-854.392578125, -808.9772338867188, 1832.6253662109375))
				case 'North':
	...
	async def cacao_pods_loop(clients: list[Client]):
		try:
			while True:
				await entity_tp_helper(foreground_client, "poopy", "Raid_AZ_Cacao", debug=False)
				await is_visible_by_path(foreground_client, ['WorldView', 'NPCRangeWin', 'imgBackground'])
				await foreground_client.send_key(Keycode.X, 0.2)
				await foreground_client.teleport(XYZ(-5003.6220703125, -1322.1357421875, 429.2038269042969))
				await is_visible_by_path(foreground_client, ['WorldView', 'NPCRangeWin', 'imgBackground'])
				await foreground_client.send_key(Keycode.X, 0.2)
				await asyncio.sleep(0.1)
	...
	async def misfortune_tears_loop(clients: list[Client]):
		try:
			while True:
				await entity_tp_helper(foreground_client, "poopy", "Raid_Jewel_Reagent", debug=False)
				await is_visible_by_path(foreground_client, ['WorldView', 'NPCRangeWin', 'imgBackground'])
				await foreground_client.send_key(Keycode.X, 0.2)
	...
	async def rope_port_hotkey():
		if foreground_client:
			logger.debug(f'Collecting Rope.')
			player_pos = await foreground_client.body.position()
			await entity_tp_helper(foreground_client, "Temp", "Raid-PL-Gear", debug=True)
			await is_visible_by_path(foreground_client, ['WorldView', 'NPCRangeWin', 'imgBackground'])
			await foreground_client.send_key(Keycode.X, 0.2)
			await foreground_client.teleport(player_pos)
	...
	async def moth_amon_hotkey():
		if foreground_client:
			...
			await entity_tp_helper(foreground_client, "Temp", "Raid_LM_Folder_Flames", debug=False)
			await is_visible_by_path(foreground_client, ['WorldView', 'NPCRangeWin', 'imgBackground'])
	...
	async def lucien_hotkey():
		if foreground_client:
			...
			await entity_tp_helper(foreground_client, "Temp", "Raid_LM_Folder_Comb", debug=False)
			await is_visible_by_path(foreground_client, ['WorldView', 'NPCRangeWin', 'imgBackground'])
			await asyncio.sleep(0.2)
			await foreground_client.send_key(Keycode.X, 0.2)
			await foreground_client.send_key(Keycode.X, 0.2)
	...
	async def morg_hotkey():
		if foreground_client:
			...
			await entity_tp_helper(foreground_client, "Temp", "Raid_LM_Folder_Mustache", debug=False)
			await asyncio.sleep(0.2)
			await foreground_client.send_key(Keycode.X, 0.2)
			await foreground_client.send_key(Keycode.X, 0.2)
	...
	async def mime_hotkey():
		if foreground_client:
			...
			await entity_tp_helper(foreground_client, "Temp", "Raid_LM_Folder_Props", debug=False)
			await is_visible_by_path(foreground_client, ['WorldView', 'NPCRangeWin', 'imgBackground'])
			await asyncio.sleep(0.2)
			await foreground_client.send_key(Keycode.X, 0.2)
			await foreground_client.send_key(Keycode.X, 0.2)
	...
	async def send_all_hotkey():
		if foreground_client:
			logger.debug(f'Sending all outside spells.')
			# Sending Mime
			await entity_tp_helper(foreground_client, "Temp", "Raid_LM_Folder_Props", debug=False)
			await is_visible_by_path(foreground_client, ['WorldView', 'NPCRangeWin', 'imgBackground'])
			await asyncio.sleep(0.2)
	...
	async def gcr_tokens():
		while wood_tokens_enabled | silver_tokens_enabled | gold_tokens_enabled:
			await gcr_chest_loop(foreground_client, wood_token, silver_token, gold_token, wood_tokens_enabled, silver_tokens_enabled, gold_tokens_enabled)


	async def toggle_wood_tokens_hotkey(token):
		global gcr_tokens_task
		global gui_send_queue
		global wood_tokens_enabled
		global wood_token
		...
		if wood_tokens_enabled:
			...
			if not (silver_tokens_enabled | gold_tokens_enabled):
				gcr_tokens_task.cancel()
				gcr_tokens_task = None
			return
	...
	async def toggle_silver_tokens_hotkey(token):
		...
		if silver_tokens_enabled:
			...
			if not wood_tokens_enabled | gold_tokens_enabled:
				gcr_tokens_task.cancel()
				gcr_tokens_task = None
			return
	...
	async def toggle_gold_tokens_hotkey(token):
		...
		if gold_tokens_enabled:
			...
			if not wood_tokens_enabled | silver_tokens_enabled:
				gcr_tokens_task.cancel()
				gcr_tokens_task = None
			return
						...
						match com.com_type:
							...
							case deimosgui.GUICommandType.ToggleOption:
								match com.data:
									...
									case GUIKeys.VVRDrums:
										await toggle_drums_hotkey(walker.clients[0], 'vvr')

									case GUIKeys.CSRDrums:
										await toggle_drums_hotkey(foreground_client, 'csr')
							...
							case deimosgui.GUICommandType.AutoChest:
								token = com.data
								await auto_chest_hotkey(token)
							...
							case deimosgui.GUICommandType.LocationTeleport:
								token = com.data
								await location_teleport_hotkey(token)

							case deimosgui.GUICommandType.SouthToken:
								token = com.data
								await south_token_teleport_hotkey(token)
							...
							case deimosgui.GUICommandType.SendAll:
								await send_all_hotkey()
