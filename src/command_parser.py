...
from src.utils import index_with_str, get_window_from_path, teleport_to_friend_from_list, auto_potions_force_buy, use_potion, is_free, logout_and_in, click_window_by_path, wait_for_visible_by_path, refill_potions, refill_potions_if_needed, wait_for_zone_change
...
from .collision_tp import WorldsCollideTP
from .packet_send import send_cantrip_packet, send_packets
    ...
    match split_command[0].lower():
        ...
        case _:
            ...
            match split_command[1].lower():
                ...
                case 'teleport' | 'tp' | 'setpos':
                    ...
                    match split_command[2]:
                        ...
                        case 'quest' | 'questpos' | 'questposition':
                            ...
                            #await asyncio.gather(*[WorldsCollideTP(p, await p.quest_position.position()) for p in clients])
                ...
                case 'writename':
                    async def forceWO(client,word):
                        windows = await client.root_window.get_windows_with_name(word)
                        if windows:
                            window = windows[0]
                            curr_flags = await window.flags() - WindowFlags.disabled
                            if curr_flags >= 0:
                                await window.write_flags(WindowFlags(curr_flags + WindowFlags.visible))
                    await asyncio.gather(*[forceWO(client, split_command[2]) for client in clients])

                case 'vodtouch':
                    async def touchstone_anywhere_button_wrapper(client, name):
                        touchstones = {
                            "Life": ["Raid_CantripObject_Life_0", MagicSchool.life],
                            "Storm": ["Raid_CantripObject_Storm_0", MagicSchool.storm],
                            "Balance": ["Raid_CantripObject_Balance_0", MagicSchool.balance],
                            "Ice": ["Raid_CantripObject_Ice_0", MagicSchool.ice],
                            "Fire": ["Raid_CantripObject_Fire_0", MagicSchool.fire],
                            "Myth": ["Raid_CantripObject_Myth_0", MagicSchool.myth],
                            "Death": ["Raid_CantripObject_Death_0", MagicSchool.death]
                        }

                        if client:
                            try:
                                sprinter = SprintyClient(client)
                                touchstone_list = await sprinter.get_base_entities_with_vague_name(touchstones[name][0])
                                if touchstone_list:
                                    touchstone = touchstone_list[0]
                                    await send_cantrip_packet(client, await client.client_object.global_id_full(), await touchstone.global_id_full(), touchstones[name][1], await touchstone.location())
                                else:
                                    print("No Touchstones")
            
                            finally:
                                pass
                    await asyncio.gather(*[touchstone_anywhere_button_wrapper(client, split_command[2]) for client in clients])
                
                case 'forcevod':
                    async def touchstone_anywhere_button_wrapper(client, name, gid):
                        touchstones = {
                            "Life": ["Raid_CantripObject_Life_0", MagicSchool.life],
                            "Storm": ["Raid_CantripObject_Storm_0", MagicSchool.storm],
                            "Balance": ["Raid_CantripObject_Balance_0", MagicSchool.balance],
                        ...
                        if client:
                            try:
                                ...
                                if touchstone_list:
                                    touchstone = touchstone_list[0]
                                    await send_cantrip_packet(client, int(gid), await touchstone.global_id_full(), touchstones[name][1], await touchstone.location())
                                else:
                                    print("No Touchstones")
            
                            finally:
                                pass
                    await asyncio.gather(*[touchstone_anywhere_button_wrapper(client, split_command[2], split_command[3]) for client in clients])
                
                case 'cannonball':
                    async def cannonballs(client):
                        await send_cantrip_packet(client, 0, 0, MagicSchool.storm, XYZ(0, 0, 0), phase=0)
                    await asyncio.gather(*[cannonballs(client) for client in clients])
                
                case 'packetsend':
                    async def send_the_packet(client):
                        await send_packets(client, debug=True)
                    await asyncio.gather(*[send_the_packet(client) for client in clients])
                ...
                case 'entitytp' | 'entityteleport':
                    ...
                    async def collide_tp_to_entity(client: Client):
                        sprinter = SprintyClient(client)
                        entities = await sprinter.get_base_entities_with_vague_name(split_command[2])
                        if entities:
                    ...
                    await asyncio.gather(*[collide_tp_to_entity(client) for client in clients])
